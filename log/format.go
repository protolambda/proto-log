// Copyright: this format.go file is based on log-15 (Apache 2.0 license)
// https://github.com/inconshreveable/log15
//
// Formatting has been adjusted to source-file info.
...
func (h *terminalHandler) format(r slog.Record) []byte {
	...
	if h.cfg.IncludeSource {
		// prepend right-padded source info
		s := r.Source()
		if s != nil {
			lineNum := strconv.FormatInt(int64(s.Line), 10)
			file, err := filepath.Rel(h.cfg.SourceRelDir, s.File)
			if err != nil {
				file = s.File
			}
			prefixLen := len(file) + 1 + len(lineNum)
			b.WriteString(file)
			b.WriteRune(':')
			b.WriteString(lineNum)
			if prefixLen < termSrcJust {
				b.Write(spaces[:termSrcJust-prefixLen])
			}
			b.WriteRune(' ')
		}
	}
...
func FormatSlogValue(v slog.Value, tmp []byte) (result []byte) {
	...
	case u256: // Need to be before fmt.Stringer-clause
...
func appendUint64(dst []byte, n uint64, neg bool) []byte {
	...
	for ; n > 0; i-- {
		if comma == 3 {
			comma = 0
			out[i] = '_' // "_" instead of "," for python use
		} else {
			comma++
			out[i] = '0' + byte(n%10)
			n /= 10
...
func appendBigInt(dst []byte, n *big.Int) []byte {
	...
	for j := len(text) - 1; j >= 0; j, i = j-1, i-1 {
		...
		case c == '-':
			buf[i] = c
		case comma == 3:
			buf[i] = '_' // "_" instead of "," for python use
			i--
			comma = 0
			fallthrough
		default:
...
type u256 interface {
	IsUint64() bool
	Uint64() uint64
	PrettyDec(separator byte) string
	Dec() string
}

// appendU256 formats n with thousand separators.
func appendU256(dst []byte, n u256) []byte {
	if n.IsUint64() {
		return appendUint64(dst, n.Uint64(), false)
	}
	res := []byte(n.PrettyDec('_')) // "_" instead of "," for python use
